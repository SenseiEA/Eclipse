...
public class LoginForm extends JFrame implements MouseListener,ActionListener{
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(ae.getSource()==btnLogin) {
			try {
				File myFile=new File("C:\\Eclipse\\LoginRegister\\bin\\Accounts.csv");
						...
						if(userName.equals(array[i]) ) {
							...
							for(int j=0;j<array.length;j++) {
								if(passwordField.equals(array[j])) {
									...
									if(userName.equals(userInput) && passwordField.equals(passwordInput)) {
									...
									else if(userName.equals(userInput) && passwordField.isEmpty()) {
										JOptionPane.showMessageDialog(null, "Enter your Password!", getTitle(), 0);
										
									}
									else if(userName.isEmpty() && passwordField.equals(passwordInput)) {
										JOptionPane.showMessageDialog(null, "Enter your Username or ID!", getTitle(), 0);
										
									}
									else if(userName.equals(userInput) && passwordField !=passwordInput) {
										JOptionPane.showMessageDialog(null, "Invalid Username or ID!", getTitle(), 0);
